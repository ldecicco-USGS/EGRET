...
flexFN <- function(
  ...
  for (seg in seq_len(nrow(dateInfo))) {
    flowIndex <- which(
      localDaily$Date >= as.Date(dateInfo[[flowStartCol]][seg]) &
        localDaily$Date <= as.Date(dateInfo[[flowEndCol]][seg])
    )

    flowNormIndex <- which(
      localDaily$Date >= as.Date(dateInfo[[flowNormStartCol]][seg]) &
    ...
    if (oldSurface) {
      ...
      concFlux_list <- getConcFluxFromSurface(
        eList,
        allLogQsByDayOfYear,
        localDaily = DailySeg,
      ...
      DailySeg$FNConc <- as.numeric(tapply(
        concFlux_list[["allConcReplicated"]],
        concFlux_list[["allDatesReplicated"]],
        "mean"
      ))
      DailySeg$FNFlux <- as.numeric(tapply(
        concFlux_list[["allFluxReplicated"]],
        concFlux_list[["allDatesReplicated"]],
        "mean"
      ))
    } else {
      DailySeg <- estDailyFromSurfaces(
        eList,
        localsurfaces = localsurfaces,
        localDaily = localDaily[flowNormIndex, ]
      )

      localDaily$ConcDay[flowIndex] <- DailySeg$ConcDay[which(
        flowNormIndex %in% flowIndex
      )]
      localDaily$FluxDay[flowIndex] <- DailySeg$FluxDay[which(
        flowNormIndex %in% flowIndex
      )]
    }

    localDaily$FNConc[flowIndex] <- DailySeg$FNConc[which(
      flowNormIndex %in% flowIndex
    )]
    localDaily$FNFlux[flowIndex] <- DailySeg$FNFlux[which(
      flowNormIndex %in% flowIndex
    )]
  }
  ...
  if ('segmentInfo' %in% names(attributes(eList$INFO))) {
    ...
    if (!is.null(customPalette)) {
      ...
      if (nrow(segmentINFO) <= 21) {
        ...
        stop(paste(
          "The number of segments exceed the length of the color palette.",
          "Supply custom palette of length",
          nrow(segmentINFO)
        ))
      }
    }
    ...
    segmentINFO[["flowNormStart"]] <- decimalDate(as.Date(
      segmentINFO[["flowNormStart"]],
      origin = "1970-01-01"
    ))
    segmentINFO[["flowStart"]] <- decimalDate(as.Date(
      segmentINFO[["flowStart"]],
      origin = "1970-01-01"
    ))
    segmentINFO[["flowNormEnd"]] <- decimalDate(as.Date(
      segmentINFO[["flowNormEnd"]],
      origin = "1970-01-01"
    ))
    segmentINFO[["flowEnd"]] <- decimalDate(as.Date(
      segmentINFO[["flowEnd"]],
      origin = "1970-01-01"
    ))
    ...
    for (i in seq_len(nrow(segmentINFO))) {
      if (showRect) {
        graphics::rect(
          segmentINFO$flowStart[i],
          par()$usr[3],
          segmentINFO$flowEnd[i],
          par()$usr[4],
      ...
      if (showArrows) {
        graphics::arrows(
          segmentINFO$flowNormStart[i],
          arrowYs[i],
          segmentINFO$flowNormEnd[i] + 1,
          arrowYs[i],
          code = 3
        )
      }
