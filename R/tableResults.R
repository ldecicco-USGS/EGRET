...
tableResults <- function(eList, qUnit = 2, fluxUnit = 9, localDaily = NA) {
  ...
  localAnnualResults <- setupYears(
    paStart = paStart,
    paLong = paLong,
    localDaily = localDaily
  )
  localAnnualResults <- localAnnualResults[
    rowSums(is.na(localAnnualResults[, c(
      "Conc",
      "Flux",
      "FNConc",
      "FNFlux"
  ...
  results <- data.frame(apply(results, 2, function(x) {
