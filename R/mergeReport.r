...
mergeReport <- function(
  ...
  if (!is.null(interactive)) {
    warning(
      "The argument 'interactive' is deprecated. Please use 'verbose' instead"
    )
    verbose <- interactive
  }
  ...
  if (exists("Daily") && !all(is.na(Daily)) && !("Q" %in% names(Daily))) {
    message(
      "Please double check that the Daily dataframe is correctly defined."
    )
  }
  ...
  if (!all(is.na(Sample))) {
    if (
      !all((c("ConcLow", "ConcHigh", "Uncen", "ConcAve") %in% names(Sample)))
    ) {
      message(
    ...
    if (!all(is.na(Daily))) {
      if (all(c("Q", "LogQ") %in% names(Sample))) {
        if (all(c("yHat", "SE", "ConcHat") %in% names(Sample))) {
          message(
            "Merging new flow data will require modelEstimation to be rerun."
          )
        }
